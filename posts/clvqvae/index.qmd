---
title: "CLVQ-VAE: Discovering Discrete Concepts Across Transformer Layers"
description: "How we use vector quantization to collapse the redundant, mixed-up features of the residual stream into a compact set of human-interpretable concepts."
date: "2026-06-11"
categories: [interpretability, LLMs, research]
image: "../../images/clvq_vae.png"
toc: true
from: markdown-smart
format:
  html:
...
This post accompanies our paper *Cross-Layer Discrete Concept Discovery for Interpreting Language Models* (with Xuemin Yu, Hassan Sajjad, and Samira Ebrahimi Kahou).
[📄 Paper](https://arxiv.org/abs/2506.20040) · [💻 Code](https://github.com/agarg-dev/CLVQVAE)
:::

## Why is it so hard to look inside a transformer?
...
1. **Features get duplicated.** A feature written at layer 3 is still visible at layers 8, 10, and 12, so a single-layer analysis sees the same concept many times over.
2. **Features get mixed.** The stream linearly superimposes many features on top of each other, so any single snapshot is a tangled sum of everything written so far.

Sparse autoencoders (SAEs) address the mixing problem, and recent cross-layer variants attack the duplication too. But SAEs operate in *continuous* space, where concepts split across many neurons without clear boundaries, and isolating one requires arbitrary activation thresholds or combinations of multiple feature directions. Humans, meanwhile, reason in discrete categories: a token either expresses *negation* or it doesn't. This mismatch limits interpretability.
...
![Overview of the CLVQ-VAE framework: lower-layer activations pass through an adaptive residual encoder, get discretized into concept vectors by the quantizer, and a transformer decoder reconstructs the higher-layer representation.](../../images/clvq_vae.png){width=85% fig-align="center"}

The framework has three crucial components.
...
![A false negative: the model misreads a positive review of a "decent imitation" as negative. The salient concept's word cloud (parody, fake, lifted, imitation, ripped) reveals that the model bound imitation-words to negative sentiment.](../../images/FN.png){width=60% fig-align="center"}
...
![A true negative: the salient concept for a correctly predicted negative review clusters negation and absence words such as nothing, barely, none, whatsoever, and hardly.](../../images/TN.png){width=60% fig-align="center"}
...
The code, scripts, and prepared datasets are available on [GitHub](https://github.com/agarg-dev/CLVQVAE). The pipeline supports RoBERTa, BERT, LLaMA-2-7B, and Qwen2.5-3B-Instruct:
...
Layer pairs used in the paper: 8 to 12 for BERT/RoBERTa, 28 to 32 for LLaMA-2-7B, 32 to 36 for Qwen2.5-3B.
